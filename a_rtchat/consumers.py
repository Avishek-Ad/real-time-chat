...
class ChatroomConsumer(WebsocketConsumer):
    def connect(self):
        ...
        # Resolve LazyUser to real DB user
        self.user = User.objects.get(id=lazy_user.id)
    ...
    def receive(self, text_data):
        ...
        event = {
            'type': 'message_handler', # to connect to message handler
            'message_id': message.id # sending through
        }
        # self.send(text_data=html) just creates one to one connection

        # for a musticast operation
        async_to_sync(self.channel_layer.group_send)(
            self.chatroom_name, event
        )

    def message_handler(self, event):
        message_id = event['message_id'] # now we have access to message id with the event object
    ...
    def update_online_count(self):
        ...
        # event is data to send browser
        event = {
            'type': 'online_count_handler', # who is going to fulfill this event
            'online_count': online_count # value to send to the event fulfiller
        }
    ...
    def online_count_handler(self, event):
        ...
        html = render_to_string("a_rtchat/partials/chat_online_tracker_p.html", context)
        self.send(text_data=html)
...
class OnlineStatusConsumer(WebsocketConsumer):
    ...
    def online_status_handler(self, event):
        ...
        context = {
            ...
            'group_chat_with_users':group_chat_with_users,
            'private_chat_with_users':private_chat_with_users
        }
